"""
============================================================
GEN 2: CaCO3 FE-SEM 다형체별 분리 학습 디퓨전 모델
============================================================
GEN 1과의 차이점:
  - 다형체(A/C/V) 라벨별로 모델을 분리해서 학습 (CLI 인자로 선택)
  - LR warmup 1,000 step 추가 (학습 초기 안정성)
  - Best moving-average loss 시점 ckpt 자동 저장 (best.pt)
    (EMA 모델의 loss가 아니라, 최근 1000 step 학습 손실 이동평균이 최저인 시점)
  - PIL 기반 grid/individual 저장으로 정확한 512x512 해상도 보존
  - 학습 진행 지표를 3종 CSV로 자동 로깅
      * train_metrics.csv   : 매 100 step
      * sample_metrics.csv  : 매 sampling 이벤트
      * eval_metrics.csv    : 학습 후 별도 evaluate.py에서 작성
  - 출력 디렉토리는 라벨별로 분리 ({output_dir}/{label}_model/)

실행 예:
  python diffusion_caco3.py --label A --device cuda:0
  python diffusion_caco3.py --label C --device cuda:1
  python diffusion_caco3.py --label V --device cuda:2
"""
...
class DiffusionTrainer:
    def __init__(self, config):
        ...
        # ───── 출력 디렉토리 (라벨별 분리) ─────
        # 예: /home/namhu/철강부산물/GEN 2/diffusion_results/A_model/
        self.output_dir = Path(config["output_dir"]) / f"{self.label}_model"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label", type=str, required=True,
                        choices=["A", "C", "V", "C+V", "others"],
                        help="학습할 라벨 (A=Aragonite, C=Calcite, V=Vaterite, "
                             "C+V=혼합, others=잔여)")
    ...
    config = {
        ...
        "num_timesteps": 1000,
        "lr": 1e-4,
        "warmup_steps": 1000,            # GEN 2 신규: LR warmup
    ...
    print("=" * 60)
    print(f"GEN 2 | DDPM + DDIM | Label='{args.label}' | Device={args.device}")
    print("=" * 60)
